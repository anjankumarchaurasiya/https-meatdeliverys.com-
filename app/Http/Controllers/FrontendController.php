...
class FrontendController extends Controller
{
    ...
    public function forgotPasswordPost(Request $request)
    {
        ...
        if($user){
            ...
            $detail['shop_name'] = CompanySetting::where('id',1)->first()->name;
           $emailsent = Mail::to($user->email)->send(new ForgetPassword($content,$detail));
           echo '<pre>';
           print_r($emailsent);
           die();
            // try{
    ...
    public function placeOrder(Request $request)
    {
        ...
        if($request->payment_type=='COD' ||$request->payment_type=='FLUTTERWAVE' ){            
        ...
        $data['currency'] = Currency::where('code',$currency_code)->first()->symbol;
        // admin notification
        $data_email = GroceryOrder::with(['customer','deliveryGuy','orderItem'])->where('id',$order->id)->first();
        $currency_email = Currency::where('code',$currency_code)->first()->symbol;
        $deliveryaddress = UserAddress::where('id',$data_email->address_id)->first();
        $details = [
           'data'=>$data_email,
           'currency'=>$currency_email,
           'deliveryaddress'=>$deliveryaddress
        ];   
        Mail::to('indscope.php@gmail.com')->send(new AdminorderList($details));
        // user notification
    ...
    public function testingofpdfgenerate()
    {

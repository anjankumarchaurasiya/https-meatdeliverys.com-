...
class GroceryController extends Controller
{
    ...
    public function createGroceryOrder(Request $request){
        ...
        foreach ($data['itemData'] as $value){
         ...
         if($notification->mail_notification==1){
             try {               
                 Mail::to($user->email)->send(new OrderCreate($content,$detail));
             } catch (\Throwable $th) {
                 //throw $th;
             }
         }
    ...
    public function singleGroceryOrder($id){
        $data = GroceryOrder::with('customer','deliveryGuy'])->find($id);
    ...
    public function paytabPayment($id){
        $order = GroceryOrder::with(['customer'])->find($id);
    ...
    public function groceryOrderRequest(){
        ...
        $pending_order = array();
        $approve_order = array();
        $pending = GroceryOrder::orWhereIn('customer_id',$radiusUser)->whereIn('shop_id',$shop)->where([['order_status','Pending'],['delivery_type','delivery']])
        ...
        $pending = GroceryOrder::whereIn('id',$pending_order)->orderBy('id', 'DESC')->get();
        foreach ($pending as $value) {
            $user= User::findOrFail($value->customer_id);
            $value->delivery_address =  UserAddress::find($value->address_id);
        }

        $approved = GroceryOrder::orWhereIn('customer_id',$radiusUser)->whereIn('shop_id',$shop)->where([['order_status','Approved'],['delivery_type','delivery']])
        ->whereDate('date',$date)->orderBy('id', 'DESC')->get();
        ...
        $approved = GroceryOrder::whereIn('id',$approve_order)->orderBy('id', 'DESC')->get();
        foreach ($approved as $value) {
            $user= User::findOrFail($value->customer_id);
            $value->delivery_address =  UserAddress::find($value->address_id);
        }
        $master['requests'] = array_merge($pending ->toArray(), $approved->toArray());
        $master['accepted'] = GroceryOrder::where([['date',$date],['deliveryBoy_id',Auth::user()->id],['order_status','DriverApproved']])

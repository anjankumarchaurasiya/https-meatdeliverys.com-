  ...
  $('.loader-container').delay('500').fadeOut(2000);
    ...
    $document.on('click', '.add-to-cart', function() {
        ...
        if(qty==1)
        {
           ...
           $('.wce.cart-'+item_id).removeClass('d-block').addClass('d-none');
            $('.wcne.cart-'+item_id).removeClass('d-none').addClass('d-block');
        }
    ...
    $document.on('click', '.buttons_added_wcne', function(event) {
        var item_id = $(this).attr("data-id");
        var qty = $(this).closest(".qty-cart").find("input[name='quantity']").val();
        var qty_ne = $(this).closest(".qty-cart").find("input[name='quantity']").attr("data-mvalue");
        var d = 1;
        if($(event.target).hasClass('minus') && qty_ne === '0')
        {
            var $this = $(".cart-close-btn-"+item_id);
            $('.wce.cart-'+item_id).removeClass('d-none').addClass('d-block');
            $('.wcne.cart-'+item_id).removeClass('d-block').addClass('d-none');

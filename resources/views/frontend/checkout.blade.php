                        ...
                        <div id="checkout_wizard" class="checkout accordion left-chck145">
                            ...
                            <div class="checkout-step">
                                <div class="checkout-card" id="headingOne">
                                    ...
                                    <h4 class="checkout-step-title"> 
                                        <button class="wizard-btn" type="button" data-toggle="collapse" data-target="#collapseFour" aria-expanded="true" aria-controls="collapseFour"> {{__('Delivery option')}} </button>
                                    </h4>
                                </div>
                                <div id="collapseFour" class="collapse in" data-parent="#checkout_wizard">
                                    <div class="checkout-step-body">
                                        ...
                                        <div class="row">
                                            ...
                                            $tomorrow = date('d-m-Y',strtotime($todaydate . "+1 days"));
                                            $datetime =strtotime($tomorrow);
                                            $wday = date('w', $today);   
                                            $datemon = date('D \<\b\\r\> d M', $today - ($wday - 1)*86400);
                                            $datetue = date('D \<\b\\r\> d M', $today - ($wday - 2)*86400);
                                            $datewed = date('D \<\b\\r\> d M', $today - ($wday - 3)*86400);
                                            $datethu = date('D \<\b\\r\> d M', $today - ($wday - 4)*86400);
                                            $datefri = date('D \<\b\\r\> d M', $today - ($wday - 5)*86400);
                                            $datesat = date('D \<\b\\r\> d M', $today - ($wday - 6)*86400);
                                            $weekdayarray[] = array('date'=>$datemon,'timestamp'=> strtotime(date('d-m-Y', $today - ($wday - 1)*86400)),'todaydate'=>$datetime); 
                                            $weekdayarray[] = array('date'=>$datetue,'timestamp'=>strtotime(date('d-m-Y', $today - ($wday - 2)*86400)),'todaydate'=>$datetime); 
                                            $weekdayarray[] = array('date'=>$datewed,'timestamp'=>strtotime(date('d-m-Y', $today - ($wday - 3)*86400)),'todaydate'=>$datetime); 
                                            $weekdayarray[] = array('date'=>$datethu,'timestamp'=>strtotime(date('d-m-Y', $today - ($wday - 4)*86400)),'todaydate'=>$datetime); 
                                            $weekdayarray[] = array('date'=>$datefri,'timestamp'=>strtotime(date('d-m-Y', $today - ($wday - 5)*86400)),'todaydate'=>$datetime); 
                                            $weekdayarray[] = array('date'=>$datesat,'timestamp'=>strtotime(date('d-m-Y', $today - ($wday - 6)*86400)),'todaydate'=>$datetime); 
                                                @endphp
                                                <div class="table-responsive">
                                                <table class="table table-bordered weekheader">
                                                    <thead>
                                                        <tr>
                                                        @php 
                                                            foreach ($weekdayarray as $key => $value) {
                                                                $checkiftrue = ($value['todaydate'] == $value['timestamp'])?'selected':'';

                                                                $datep = '<th class="'.$checkiftrue.'" >'.$value['date'].'</th>';  
                                                                echo $datep;
                                                            }  
                                                        @endphp
                                                        </tr>
                                                    </thead>
                                                    <tbody class="weekboady">
                                                        <tr>
                                                            <td colspan="6" class="deliverytime">
                                                                <input type="radio" checked="" name="deliver_time"  > <span>9AM - 1 PM</span>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>

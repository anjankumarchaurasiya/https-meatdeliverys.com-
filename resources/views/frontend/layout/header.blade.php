...
<div class="modal d-block" tabindex="-1" role="dialog" style="background: rgba(0, 0, 0, 0.3);">
  <div class="modal-dialog modal-dialog-centered rt-lgoinmodal" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title text-center">Please select your delivery location</h5>
        
      </div>
      <div class="modal-body">
        <!--  -->
         <div class="row text-center" style="min-height: 200px;">
            <div class="select_location popup">
               <i class="uil uil-location-point"></i>
               <select class="text-left select2" onchange="if (this.value) window.location.href=this.value">
                   <option value="" >Location</option>
                   @foreach ($locations as $item)
                    @php 
                   $selected_ifyes = (session()->exists('location_id')  && session('location_id') == $item->id)?'selected':'';
                   @endphp
                   <option  value="{{ url('setLocation/'.$item->id) }}" {{$selected_ifyes}}>{{$item->name}}</option>
                   @endforeach
               </select>     
            </div>
         </div>
    ...
    <div class="top-header-group">
        <div class="top-header">
            ...
            <div class="select_location">
               <i class="uil uil-location-point"></i>
               <select class="select2" onchange="if (this.value) window.location.href=this.value">
                   <option value="" >Location</option>
                   @foreach ($locations as $item)
                    @php 
                   $selected_ifyes = (session()->exists('location_id')  && session('location_id') == $item->id)?'selected':'';
                    ...
                    @if (Auth::check())
                        ...
                        <li class="ui dropdown">
                            ...
                            <div class="menu dropdown_account">
                                ...
                                <a href="{{url('/profile/change-password')}}" class="item channel_item"><i class="uil uil-padlock    icon__1"></i> {{__('Chnage Password')}} </a>
                                ...
                                <a href="{{ route('logout') }}"  onclick="event.preventDefault(); document.getElementById('frm-logout').submit();" class="item channel_item"><i class="uil uil-lock-alt icon__1"></i>Logout</a>
